...
const MOCK_DATA: Array<DataItem> = [
  {
    title: "Level up",
...
const getDataItems = (
  ...
  // Phase 1: refine the result set based on the provided search string
  ...
  const filteredData =
    normalizedSearchString.length > 0
      ? MOCK_DATA.filter((item) => {
          return (
            item.title.toLowerCase().includes(normalizedSearchString) ||
            item.description.toLowerCase().includes(normalizedSearchString)
          );
        })
      : MOCK_DATA;

  // Phase 2: return a paginated fragment of the result set

  if (offset >= filteredData.length) {
    return Promise.resolve({ items: [], totalItems: filteredData.length });
  }

  const cappedLimit =
    offset + limit > filteredData.length ? filteredData.length : offset + limit;

  return new Promise((resolve) => {
    setTimeout(
      () =>
        resolve({
          items: filteredData.slice(offset, cappedLimit),
          totalItems: filteredData.length,
        }),
      Math.random() * 1000,
    ); // Simulate server latency/op cost
  });
};

export { getDataItems };
export type { DataItem, DataReqResult };
